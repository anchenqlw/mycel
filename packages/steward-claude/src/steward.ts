...
export class ClaudeSteward implements StewardPort {
  ...
  async repair(input: StewardPlanInput, diagnostics: PlanDiagnostic[], previous: HarnessIntent, onProgress?: (phase: "inspecting-resources") => void | Promise<void>): Promise<HarnessIntent> {
    return this.#invoke([
      ...
      "Repair the response exactly once. Return a corrected proposal when all diagnostics can be resolved from known context; otherwise return one clarification question and do not guess.",
...
export function stewardPrompt(input: StewardPlanInput): string {
  return [
    ...
    "A schedule must include intervalMs, HH:mm timeOfDay, and an IANA timezone. When the user gives a local clock time without another timezone, use the provided Local IANA timezone without asking. Ask about timezone only when the user explicitly needs a different location, the location is materially ambiguous, or no valid local timezone is available.",
